...
class Browser:
    ...
    def call(self, method, params=None):
        """One DevTools call, answered with its result."""
        self.next_id += 1
        wanted = self.next_id
        self._send({"id": wanted, "method": method, "params": params or {}})
        while True:
    ...
    def photograph(self, path):
        ...
        with open(path, "wb") as out:
            out.write(base64.b64decode(result["data"]))
        return path

    def click(self, x, y):
        """A real click on the page, where the window is being served.

        The window is a canvas the browser is holding, and broadway turns the
        browser's own pointer events into the ones GTK delivers, so this is a
        click on the widget under that point rather than a method call standing
        in for one.
        """
