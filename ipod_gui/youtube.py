...
def fetch_command(url, output, new_tracks=None, single=True):
    ...
    One builder keeps the shared flags consistent for a search result's Add,
    adding one to a playlist, the Add all beside a pasted playlist and a
    preview, while letting each supply its output, new-track manifest and
    playlist scope. Add all is the only one that asks for the whole list; every
    other caller takes the single video a link names.
    """
