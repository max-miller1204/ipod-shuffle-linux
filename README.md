...
An album has its tracks' art, and a playlist normally borrows the first cover the tracks it names carry - out of your library for a playlist made here, and out of what was read off the device for one that exists only there.
A playlist made here can override that choice with a custom JPEG, PNG or WebP image copied into `~/Music/Playlists/.covers`.
Only a list with neither a custom image nor a song carrying a cover falls back to the placeholder its name generates, which is the same placeholder every time that list is drawn.

None of it reaches the iPod.
The custom playlist image remains on this computer, the playlist M3U contains only track paths, and `ipod-fetch.sh` still passes `--no-embed-thumbnail` because cover art is pure waste on a 2GB device with no screen.
...
Library results include local, preview and connected-device states, while a search answers from the configured music folders alone; cache pruning and clearing are restricted to the application's configured preview cache.
...
`tests/scratch.py` hands out the temporary directories the checks build their fixtures in - a sandbox `HOME`, a stand-in iPod, a library to scan - and removes each one when the check that asked for it ends, on every path out rather than only the ones a check returns through.
Set `IPOD_KEEP_SCRATCH=1` to keep them and have each location printed, for looking at what a check actually built.
...
A sync is held to that same reading although it deletes nothing, since it still copies tracks and rebuilds the database: a token that is not its own plan's, whether invented, made for other sources, or made before the options saved on the device changed, has to leave both the volume and the database builder's record exactly as they were.
